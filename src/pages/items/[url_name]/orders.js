...
function Orders() {
  ...
  const addToFavorites = async (itemDetail) => {
    if (!session) return alert('You need to be logged in to add to favorites');

    console.log('Item ID:', itemDetail.itemDetail.id); 
    ...
    try {
      const response = await fetch('/api/favorites/add', {
        ...
        body: JSON.stringify({
          ...
          itemId: itemDetail.itemDetail.id,
          itemName: url_name,
    ...
    <table className='min-w-full divide-y divide-gray-200'>
      <thead className='bg-gray-50'>
        <tr>
          ...
          <th className='pl-6 text-center text-xs font-medium text-gray-500 uppercase tracking-wider'>Buy Item</th>
          ...
          <>
            ...
            {filteredItems .sort((a, b) => a.platinum - b.platinum) .map((itemDetail) => (
                <tr key={itemDetail.id}>
                  ...
                  <td onClick={() => handleCopyClick(itemDetail)} className={`pl-6 whitespace-nowrap text-sm cursor-pointer hover:underline text-center ${itemDetail.mod_rank === 5 ? 'bg-green-50' : ''}`}>Buy</td>
                  <td className={`pl-6 whitespace-nowrap text-sm cursor-pointer hover:underline text-center ${itemDetail.mod_rank === 5 ? 'bg-green-50' : ''}`} onClick={() => addToFavorites({ id: 'item-id' ,itemDetail })}>Add</td>
                </tr>
              ))}
            </tbody>
          </>
